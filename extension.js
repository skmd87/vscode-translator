...
function activate(context) {
	...
	let disposableNew = vscode.commands.registerCommand('translator.new', newTranslation);
	...
	if (editor) {
		...
		getTranslations(editor.document);
	}
	...
	let disposable3 = vscode.window.onDidChangeActiveTextEditor(editor => {
		if (!editor) {
			return;
		}
		let document = editor.document;
		if (document.languageId === "vue") { // check if document is a .vue file
			highlightMatches(document);
		}
	});
...
async function newTranslation() {
	...
	// if translation is surrounded by quotes, prompt the user to remove them using showQuickPick
	if (translation && (translation.startsWith('"') || translation.startsWith("'"))) {
		const options = [
			'Yes',
			'No'
		]
		await vscode.window.showQuickPick(options, {
			placeHolder: 'Would you like to remove quotes (")?',
...
async function getTranslations(document) {
...
async function login() {
	...
	if (!config.get('isLoginRequired') || token) {
		return true;
	}
	...
	if (!token) {
		...
		await api.post(loginURL,
			...
			debugChannel.appendLine('New token:' + token);
...
function highlightMatches(document) {
	...
	if (editor && editor.document === document) {
		...
		while (match = regex.exec(text)) {
			let startPos = document.positionAt(match.index + (match[0].startsWith('$t') ? 4 : 2)); // start after "$t(" or "t("
			let endPos = document.positionAt(match.index + match[0].length - (match[0].endsWith(')') ? 2 : 1)); // end before the closing parenthesis or the comma (Vue 3 syntax)
